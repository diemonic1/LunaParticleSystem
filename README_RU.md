...
5) После настройки системы частиц необязательно переносить отдельные значения каждого измененного поля вручную. У каждой системы частиц есть кнопка копирования настроек. При ее нажатии в буфер обмена копируется JSON строка со всем настройками. Вы можете вставить ее в соответствующее поле на компоненте `LunaParticleSystem`, и после нажати кнопки Apply JSON все параметры применяться к системе частиц автоматически.
